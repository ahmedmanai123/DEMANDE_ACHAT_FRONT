...
import { AFFECTATION_DEMANDEDto, DA_BESOIN_AACHETERDto, DA_BESOIN_ARTICLEDto, DA_BESOINDto, DA_TRACABILITEBESOINARTICLEDto, DA_VALIDATION, GridResponse, Type_Validation, TypeSage, Validateur_BesoinVM } from "@/types/besoin.types";
...
export const besoinService = {
  ...
  // GET /api/besoin/{id}
  getById: (id: number): Promise<DA_BESOINDto> =>
    api.get(`/besoin/${id}`).then((r) => r.data),
  ...
  getLastNumero: (): Promise<{ numero: string }> =>
    api.get("/besoin/last-numero").then((r) => r.data),
  ...
  exportExcel: async (filter: Partial<DA_BESOINDto>): Promise<void> => {
    ...
    const fileName =
      response.headers["file-name"] ?? "Liste_Demandes_Besoin.xlsx";
...
export const articleBesoinService = {
  ...
  getAll: (
    filter: Partial<DA_BESOIN_ARTICLEDto>
  ): Promise<GridResponse<DA_BESOIN_ARTICLEDto>> =>
    api.get("/besoin/articles", { params: filter }).then((r) => r.data),

  // GET /api/besoin/{id}/articles
  getByBesoinId: (besoinId: number) =>
    api.get(`/besoin/${besoinId}/articles`).then((r) => r.data),

  // POST /api/besoin/articles
  save: (article: Partial<DA_BESOIN_ARTICLEDto>) =>
    api.post("/besoin/articles", article).then((r) => r.data),

  // DELETE /api/besoin/articles/{bA_No}
  delete: (bA_No: number) =>
    api.delete(`/besoin/articles/${bA_No}`).then((r) => r.data),
...
export const validationService = {
  ...
  getStatutUser: (b_No: number, type: Type_Validation): Promise<number> =>
    api
      .get(`/besoin/${b_No}/statut-user`, { params: { type } })
      .then((r) => r.data),

  // GET /api/besoin/{id}/validation
  getValidationView: (id: number, b_No: number) =>
    api
      .get(`/besoin/${id}/validation`, { params: { b_No } })
      .then((r) => r.data),

  // POST /api/besoin/valider-etape
  validerEtape: (validation: DA_VALIDATION) =>
    api.post("/besoin/valider-etape", validation).then((r) => r.data),
  ...
  getHistorique: (
    ...
    api
      .get("/besoin/historique-validation", { params: { ...filter, v_Type } })
      .then((r) => r.data),
...
export const gestionDemandeService = {
  ...
  getDetailsDemande: (id: number) =>
    api.get(`/besoin/demandes-aacheter/${id}`).then((r) => r.data),
  ...
  getTableauxComparatifs: (b_No: number) =>
    api.get(`/besoin/${b_No}/tableaux-comparatifs`).then((r) => r.data),
...
export const referenceService = {
  getDepotsAutoriser: (bT_Id: number, b_IdDemandeur: string) =>
    api
      .get("/besoin/depots-autoriser", { params: { bT_Id, b_IdDemandeur } })
      .then((r) => r.data),

  getAffairesAutoriser: (bT_Id: number) =>
    api.get("/besoin/affaires-autoriser", { params: { bT_Id } }).then((r) => r.data),

  getArticleDivers: (bT_Id: number) =>
    api.get("/besoin/article-divers", { params: { bT_Id } }).then((r) => r.data),

  getInfoTypeBesoin: (bT_Id: number) =>
    api.get("/besoin/info-type-besoin", { params: { bT_Id } }).then((r) => r.data),

  getChampsLibresBesoin: (bT_Id: number, b_No: number) =>
    api
      .get("/besoin/champs-libres/besoin", { params: { bT_Id, b_No } })
      .then((r) => r.data),
};