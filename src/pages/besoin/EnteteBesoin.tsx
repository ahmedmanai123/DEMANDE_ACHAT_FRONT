	...
	return (
		<div className="flex items-center gap-3">
			<div className="relative flex-1 h-2 bg-gray-200 rounded-full">
				<div className="absolute h-2 rounded-full bg-blue-500 transition-all" style={{ width: `${percent}%` }} />
			</div>
			<span className={`text-sm font-bold min-w-[90px] ${info.color}`}>{info.label}</span>
		</div>
	);
};
...
const ReadField: React.FC<{
	...
	<div className="form-group">
		{linkTo ? (
			<label className="col-form-label" style={{ color: "#03a9f3", cursor: "pointer" }}>
				{/* Remplace : $(#lbDemandeBesoin).click → window.open */}
				<Link to={linkTo} target="_blank" rel="noreferrer">
					{label}
				</Link>
			</label>
		) : (
