...
const ValidationAction: React.FC<Props> = ({
	...
	const isEnAttente = validation.V_Status === Statut.EnAttente || validation.V_Status === Statut.Attente_Acheter;

	const isDemandeur = validation.V_Role_Validateur === Role_Validateur.Demandeur;
	...
	return (
		<div className="flex flex-wrap gap-2">
			{/* ── Bouton Valider ── */}
			{canValider && (
				<Popconfirm title="Confirmer la validation ?" onConfirm={handleValider} okText="Oui" cancelText="Non">
					...
					>
						{motifsRefus.length > 0 && (
							<div className="mb-3">
								...
								<Select style={{ width: "100%" }} placeholder="Choisir un motif..." onChange={(v) => setMR_No(v)}>
									{motifsRefus.map((m) => (
