...
const ValidateurBesoin: React.FC<Props> = ({ bT_Id, idBesoin, idDemandeur, v_Type }) => {
	const { validateurs, loading } = useCircuitValidation(bT_Id, idBesoin, idDemandeur, v_Type);
	...
	const columns: ColumnsType<Validateur_BesoinVM> = [
		{
			...
			render: (name: string) => (
				<div className="flex items-center gap-2">
					<Avatar size={32} style={{ backgroundColor: getColorFromName(name ?? ""), fontSize: 13 }}>
						{getInitials(name ?? "")}
					</Avatar>
